...
async function registerUsers() {
  ...
  await Promise.all(users.map(async (user, id) => {
    await client.$withUser(user.identity).register(new User({ id: id.toString(), name: user.name }))
...
registerUsers()
  .then(() => client.$withUser(users[0].identity).rename('0', 'Armando'))
  .then(async () => log('User Renamed', await User.getOne('0')))
